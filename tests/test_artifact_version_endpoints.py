    ...
    with Session(engine) as session:
        deleted_event = next(
            ...
            for event in session.exec(
                select(ArtifactActivityEvent).where(ArtifactActivityEvent.event_type == "deleted")
            ).all()
            if event.details.get("path") == str(folder.resolve())
        )
        artifact = session.get(Artifact, deleted_event.artifact_id)
