...
async def upload_file(
    ...
    return await FileService(scoped).create_file(file, purpose)


@router.get("/", response_model=FileListResponse)
def list_files(scoped: ScopedSessionDep, purpose: str | None = None):
    return FileListResponse(data=FileService(scoped).list_files(purpose=purpose))


@router.get("/{file_id}", response_model=FileResponse)
def retrieve_file(file_id: UUID, scoped: ScopedSessionDep):
    try:
        return FileService(scoped).get_file(file_id)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))


@router.delete("/{file_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_file(file_id: UUID, scoped: ScopedSessionDep):
    found = FileService(scoped).delete_file(file_id)
    if not found:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="File not found")


@router.get("/{file_id}/content")
def retrieve_file_content(file_id: UUID, scoped: ScopedSessionDep):
    try:
        content_type, filename, path = FileService(scoped).get_file_content(file_id)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
