...
def reconcile_skill_links(skill: Skill) -> None:
    ...
    canon = safe_join(_canon_root(), skill.name)
    ...
    for project_dir in all_projects:
        link = safe_join(project_dir / "skills", skill.name)
...
def remove_skill_links(slug: str) -> None:
    ...
    for project_dir in _project_dirs():
        _remove_link(safe_join(project_dir / "skills", slug))
...
def reconcile_project(project_dir: Path, skills: list[Skill]) -> None:
    ...
    for skill in skills:
        ...
        if skill.projects and project_dir.name not in skill.projects:
            continue
        canon = safe_join(canon_root, skill.name)
        if canon.exists():
            _ensure_symlink(safe_join(project_dir / "skills", skill.name), canon)
