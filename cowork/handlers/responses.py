...
def _auth_failure_provider(settings, role: str | None) -> Provider | None:
    ...
    if role == "router":
        # Defensive, and not reachable today: both router call sites swallow a
        # confirmed refusal rather than propagating it — anton's `summarize()`,
        # whose `test_failed_summarize_reports_no_compaction` pins the fallback,
        # and `_route_decision` below. Kept so a future propagating router path
        # attributes the card to the provider that actually failed rather than
        # defaulting to planning.
