...
class UserSettings(Settings):
    ...
    def _resolve_provider(self, preferred: Provider) -> Provider:
        # Keep the selected provider when it has credentials; otherwise fall back
        # to a configured one so a usable key still drives the agent even when
        # the selected provider was never set up (e.g. the default MindsHub is
        # selected but only an Anthropic key exists). Prefers MindsHub, then any
        # configured provider. Returns the original when nothing is configured so
        # the caller still surfaces the not-ready state.
